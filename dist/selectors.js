...
export function stripGenericSuffixes(intent) {
    const words = intent.trim().split(/\s+/);
    if (words.length <= 1)
        return null;
    let end = words.length;
    while (end > 0 && GENERIC_SUFFIXES.has(words[end - 1].toLowerCase()))
        end--;
    if (end === 0)
        return null; // every word is generic, nothing distinctive left
    if (end === words.length)
        return null; // nothing was stripped, no new attempt to make
 ...
 * no extra cost.
 */
export async function resolve(page, spec) {
 ...
 */
async function resolveInFrames(page, scope, spec, chain, depthLeft) {
    if (depthLeft <= 0)
        return null;
 ...
 * emitted spec.
 */
async function enumerateFrameSelectors(scope) {
    const sels = [];
...
export function frameLocatorForChain(page, chain) {
    let fl = page.frameLocator(chain[0]);
    for (let i = 1; i < chain.length; i++)
        fl = fl.frameLocator(chain[i]);
    return fl;
}
/**
 ...
 */
export function parsePiercingSelector(css) {
 ...
 * needed.
 */
async function resolveInScope(page, spec) {
    const role = spec.role ?? guessRole(spec.intent);
    const name = spec.label ?? spec.intent;
    const ambiguousCandidates = [];
    const tryCandidate = async (c) => {
        const n = await countOf(c.locator);
        if (n === 1)
            return { locator: c.locator, level: c.level, arg: c.arg, ambiguous: false };
        if (n > 1)
    ...
    //    then a generic-suffix-stripped form for each.
    {
        ...
        const addPh = (s) => {
            if (s && !placeholderCandidates.includes(s))
                placeholderCandidates.push(s);
        };
        addPh(spec.label);
        addPh(spec.intent);
    ...
    //    matching unrelated elements; intent is tried as a last resort below.
    {
        ...
        const addTxt = (s) => {
            const t = s?.trim();
            if (t && t.length >= 3 && !textCandidates.includes(t))
    ...
    // 5. getByAltText: for images with alt text. Tries label hint, then intent.
    {
        const altCandidates = [];
        const addAlt = (s) => {
            if (s && !altCandidates.includes(s))
                altCandidates.push(s);
        };
        addAlt(spec.label);
        addAlt(spec.intent);
    ...
    //    Tries label hint, then intent.
    {
        ...
        const addTitle = (s) => {
            if (s && !titleCandidates.includes(s))
                titleCandidates.push(s);
        };
        addTitle(spec.label);
    ...
    if (ambiguousCandidates.length > 0) {
        const priority = ['role', 'label', 'placeholder', 'text', 'alt', 'title', 'testid', 'css', 'xpath'];
 ...
 */
async function tryRoleNameVariants(page, role, name, tryCandidate) {
    const r = role;
 ...
 */
export function normalizeCssQuotes(css) {
    if (!css || (css.indexOf('\\"') < 0 && css.indexOf("\\'") < 0))
        return css;
 ...
 */
function buildSmartCssFromIntent(intent) {
    const lc = intent.toLowerCase();
    if (/\bpassword\b/.test(lc))
        return 'input[type="password"]';
    if (/\bemail\b/.test(lc))
        return 'input[type="email"]';
    if (/\bsearch\b/.test(lc))
        return 'input[type="search"]';
    if (/\b(phone|telephone|tel)\b/.test(lc))
        return 'input[type="tel"]';
    if (/\burl\b/.test(lc))
        return 'input[type="url"]';
    if (/\bsubmit\b/.test(lc))
 ...
 */
export function frameScopeExpr(frameChain) {
 ...
 */
export function emitLocatorCall(level, arg, ambiguous = false, frameChain) {
    ...
    switch (level) {
        case 'role': {
            const a = arg;
            if (!a.name)
                return `${root}.getByRole(${JSON.stringify(a.role)})${tail}`;
            const opts = { name: a.name };
            if (a.exact)
                opts.exact = true;
            return `${root}.getByRole(${JSON.stringify(a.role)}, ${JSON.stringify(opts)})${tail}`;
        }
        case 'label':
            return `${root}.getByLabel(${JSON.stringify(arg)})${tail}`;
        case 'placeholder':
            return `${root}.getByPlaceholder(${JSON.stringify(arg)})${tail}`;
        case 'text':
            return `${root}.getByText(${JSON.stringify(arg)})${tail}`;
        case 'alt':
            return `${root}.getByAltText(${JSON.stringify(arg)})${tail}`;
        case 'title':
            return `${root}.getByTitle(${JSON.stringify(arg)})${tail}`;
        case 'testid':
            return `${root}.getByTestId(${JSON.stringify(arg)})${tail}`;
        case 'css':
            return `${root}.locator(${JSON.stringify(arg)})${tail}`;
        case 'xpath':
            return `${root}.locator(${JSON.stringify(`xpath=${arg}`)})${tail}`;
    }
}
