...
function parseCliArgs(argv) {
    ...
    const out = { dryRun: false, apply: false, yes: false, json: false };
    for (let i = 0; i < args.length; i++) {
        ...
        if (a === '--config')
            ...
            if (!Number.isInteger(n) || n < 0)
                fail('--max-heals expects a non-negative integer.');
            out.maxHeals = n;
        }
        else if (a === '--verify')
            out.verify = true;
            ...
            out.verify = false;
        else if (!a.startsWith('--') && !out.specPath)
            out.specPath = a;
        else
            fail(`Unknown argument: ${a}`);
    }
    if (out.dryRun && out.apply)
        fail('Pass either --dry-run or --apply, not both.');
    return out;
}
/** Every *.spec.ts / *.spec.js under dir, recursive, sorted for determinism. */
function findSpecs(testDir) {
    const out = [];
    const walk = (d) => {
        let entries;
        try {
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory())
                walk(p);
            else if (/\.spec\.(ts|js)$/.test(e.name))
                out.push(p);
        }
    };
    walk(testDir);
 ...
 */
async function confirmApply(cli, count) {
    if (cli.yes)
        return true;
    ...
    console.error('Applying heals requires confirmation. Preview with --dry-run, or add --yes to apply non-interactively.');
    process.exit(1);
}
/** Nearest directory at or above the spec that holds a package.json. */
function projectRootFor(specPath) {
...
async function main() {
    ...
    const fromCfg = (p) => path.resolve(cfgDir, p);
    let specs = [];
    if (cli.specPath)
        specs = [path.resolve(cli.specPath)];
    else if (cfg.testDir)
        specs = findSpecs(fromCfg(cfg.testDir));
    if (specs.length === 0) {
    ...
    const allowedLevels = cfg.selectorPreference;
    ...
    const say = (line) => { if (!cli.json)
        console.log(line); };
    const eventPrinter = (e) => {
        switch (e.type) {
            case 'scanned':
                say(`  · scanned ${e.total} locator(s) across ${e.files} file(s)`);
                break;
            case 'opened_page':
                say(`  · opened ${e.url}`);
                break;
            case 'healing':
                say(`  → broken: ${e.selector}`);
                break;
            case 'healed':
                say(`  ✓ healed to ${e.new}  (level=${e.level})`);
                break;
            case 'unhealed':
                say(`  ✗ unhealable: ${e.selector}\n      ${e.reason}`);
                break;
            case 'done':
                say(`  ${e.intact} intact · ${e.healed} healed · ${e.unhealed} unhealable (of ${e.total} scanned)\n`);
                break;
            default: break;
        }
    };
    const runPass = async (writePass, silent) => {
        const locs = [];
        const per = [];
        for (const spec of specs) {
            if (!silent)
                say(`▸ Healing ${path.relative(process.cwd(), spec)}${writePass ? '' : '  (dry run, no files written)'}`);
    ...
    if (cli.json) {
        const count = (s) => locators.filter((l) => l.status === s).length;
        const payload = {
            dryRun: !write,
